...
class ViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    static let ip: String = "http://169.254.247.36:3000"

    let socket = SocketIOClient(
        socketURL: URL(string: ViewController.ip)!,
    ...
    override func viewDidLoad() {
        ...
        let tapGestureRecognizer = UITapGestureRecognizer(
            target: self,
            action: #selector(ViewController.userTapped(_:)))
        view.addGestureRecognizer(tapGestureRecognizer)

        resetButton.addTarget(
            self,
            action: #selector(ViewController.resetTapped(_:)),
            for: .touchUpInside)
    ...
    func sendXValue(_ value: Int) {
        socket.emit("x", value)
    }

    func sendYValue(_ value: Int) {
        socket.emit("y", value + 200)
    }

    func synthTrigger(_ isOn: Bool) {
        socket.emit("synthTrigger", isOn)
    }

    func pitchForFrequency(frequency: Double) -> Int {
        return Int(round(12 * log2(frequency / 8)))
    }
    ...
    private func handleVisionRequestUpdate(_ request: VNRequest, error: Error?) {
        ...
        DispatchQueue.main.async {
            ...
            let xValue = Int(convertedRect.origin.x)
            let yValue = Int(convertedRect.origin.y)

            self.sendXValue(xValue)
            self.sendYValue(yValue)
        }
    }
